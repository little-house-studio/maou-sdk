...
function writeNodeLauncher(name, jsPath, envName) {
  mkdirSync(BIN_DIR, { recursive: true });
  if (IS_WIN) {
    const cmd = `@echo off
setlocal
set "CLI_JS=%${envName}%"
if "%CLI_JS%"=="" set "CLI_JS=${jsPath}"
if not exist "%CLI_JS%" (
  echo ${name}: entry not found: %CLI_JS% 1>&2
...
exit /b %ERRORLEVEL%
`;
    writeFileSync(join(BIN_DIR, `${name}.cmd`), cmd, "utf-8");
    log(`  ${join(BIN_DIR, `${name}.cmd`)} ✓`);
    return;
  }
  const sh = `#!/bin/sh
# generated by scripts/setup-dev.mjs — 指向源码树，改代码后 pnpm -r build 即生效
CLI_JS="\${${envName}:-${jsPath}}"
if [ ! -f "$CLI_JS" ]; then
  echo "${name}: entry not found: $CLI_JS" >&2
  echo "  re-run: pnpm setup:dev" >&2
  exit 127
fi
command -v node >/dev/null 2>&1 || { echo "${name}: need Node >= 20 on PATH" >&2; exit 127; }
PATH="${vendorBinDir(REPO_ROOT)}:$PATH"
export PATH
exec node "$CLI_JS" "$@"
`;
  const wrap = join(BIN_DIR, name);
...
/** ~/.maou/bin/maou + aiinstall → 本源码树 */
function linkLauncher() {
  if (has("no-link")) return;
  step("链接 maou / aiinstall → 本源码树");
  const cliDist = join(REPO_ROOT, "cli", "dist", "index.js");
  const installDist = join(REPO_ROOT, "agent-products", "install-agent", "dist", "bin.js");
  if (!existsSync(cliDist)) {
    warn(`${cliDist} 不存在（构建未完成？）—— 跳过链接`);
    return;
  }
  writeNodeLauncher("maou", cliDist, "MAOU_CLI_JS");
  if (existsSync(installDist)) {
    writeNodeLauncher("aiinstall", installDist, "MAOU_AIINSTALL_JS");
  } else {
    warn(`${installDist} 不存在 —— 跳过 aiinstall 链接（先 pnpm --filter @little-house-studio/install-agent build）`);
  }
...
log("  maou setup       配置 API（需要交互式 TTY；非 TTY 会跳过，不是安装失败）");
  ...
  log("  aiinstall <url>  安装员（无 TUI；完整版会一并挂上）");
