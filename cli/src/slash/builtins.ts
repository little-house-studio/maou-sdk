...
export const BUILTIN_CLI_COMMANDS: readonly CliCommandSpec[] = [
  ...
  },
  {
    ...
    description: "调试 system / 文件缓存区 / tools / 上下文动态区（不进上下文）",
    scope: "local",
    category: "debug",
    palette: true,
  ...
  // ── runtime fallbacks（agent commandRegistry 未就绪时补全仍可用）──
  {
    ...
    name: "plan",
    label: "计划模式",
    description: "先调查并写计划，确认后再改代码",
    usage: "[<objective>|off|status|view|approve|revise <notes>|clear]",
    scope: "runtime",
    category: "agent",
    palette: false,
    args: [{ name: "objective", description: "计划任务，或 off/status/view/approve/revise/clear", rest: true }],
  },
  {
    id: "goal",
    name: "goal",
    aliases: ["easygoal"],
    label: "目标模式",
    description: "同会话长目标：goal 模式续跑，按完成度收口",
    usage: "[<objective>|clear|edit <objective>|pause|resume]",
    scope: "runtime",
    category: "agent",
    palette: false,
    args: [{ name: "objective", description: "目标正文，或 clear/edit/pause/resume", rest: true }],
  },
  {
    id: "ultragoal",
    name: "ultragoal",
    label: "Ultra 目标",
    description: "多个 agent 宿主编排的长目标：规划、评审、验审",
    usage: "[<objective> [--budget <tokens>] | status | pause | resume | clear]",
    scope: "runtime",
    category: "agent",
    palette: false,
    args: [{ name: "objective", description: "目标正文，或 status/pause/resume/clear；可加 --budget <tokens>", rest: true }],
  },
  {
    id: "stop",
    name: "stop",
  ...
  },
  {
    ...
    aliases: ["terminals"],
    label: "终端会话",
    description: "列出/查看/停止与 WebUI 共用的终端（不嵌 PTY）",
    usage: "[list] | logs <id> | stop <id>",
    scope: "local",
    category: "system",
    palette: true,
    args: [
      { name: "sub", description: "list | logs | stop", required: false },
      { name: "id", description: "终端 id", required: false },
    ],
    local: { kind: "action", action: "term_sessions" },
  },
  {
    id: "init",
    name: "init",
