 ...
 * useExitGuard —— 退出安全网。
 *
 * 退出只做显光标 + 关 raw mode，不写备用屏/鼠标退出序列（它从不开这些）。
...
function restore(): void {
  ...
  // or racing exit can corrupt the TTY. Only restore when held the screen.
  const tui = (
...
export function installExitGuard(): () => void {
  ...
  const onSignal = (sig: NodeJS.Signals) => {
    restore();
    // 直接强退，不等 的 waitUntilExit resolve——LLM fetch / 定时器可能让它永不 resolve（假退出）。
    // restore 已写终端恢复序列，安全。
