 ...
 * 逐终端降级：iTerm2/Alacritty/kitty/Ghostty/WezTerm 写；XTerm/Apple Terminal 无操作。
 * SSH 环境下 Claude Code 也用此机制（带平台回退，这里简化只发 OSC52）。
 */

/** 检测终端是否支持 OSC52（保守：已知支持的终端才写） */
export function osc52Supported(): boolean {
  const tp = process.env.TERM_PROGRAM ?? "";
  const term = process.env.TERM ?? "";
  // 已知支持
  if (["iTerm.app", "WezTerm", "ghostty", "kitty"].includes(tp)) return true;
