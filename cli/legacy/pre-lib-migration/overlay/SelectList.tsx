...
export function SelectList({ items, onSelect, visibleCount = 8, innerWidth = 42 }: Props) {
  ...
  // 滚轮驱动菜单滚动（overlayScrollCmd nonce 变化即触发）
  useEffect(() => {
  ...
  return (
    <Box flexDirection="column">
      {visible.map((it, i) => {
        ...
        return <SelectRow key={it.value} item={it} isSel={isSel} onClick={() => onSelect(it.value)} />;
      })}
    </Box>
  );
}

function SelectRow({ item, isSel, onClick }: {
  item: SelectItem; isSel: boolean; onClick: () => void;
}) {
  const t = useTheme();
