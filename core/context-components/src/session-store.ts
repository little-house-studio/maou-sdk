...
import { maouSessionLayout } from "./factory/layout.js";
import {
  resolveSessionFeatures,
  type SessionLayout,
  type SessionRecordFeatures,
  type SessionRecordOptions,
} from "./factory/ports.js";
...
export class SessionStore {
  ...
  readonly features: Required<SessionRecordFeatures>;
  readonly searchIndex: SessionSearchIndex;
  private agentNameCache = new Map<string, string>();
  private liveIds = new Set<string>();

  constructor(sessionDir: string, options?: SessionRecordOptions) {
    this.sessionDir = sessionDir;
    this.layout = options?.layout ?? maouSessionLayout(sessionDir);
    this.features = resolveSessionFeatures(options?.features);
    mkdirSync(sessionDir, { recursive: true });
    this.searchIndex = new SessionSearchIndex(sessionDir, { enabled: this.features.search });
    setLedgerAppendListener(sessionDir, (info) => this.handleLedgerAppend(info));
  }

  private branchOf<T extends TreeFields>(messages: T[], leafId?: string | null): T[] {
    if (!this.features.tree) return messages;
    return selectBranch(messages, leafId);
  }
  ...
  search(opts: {
    ...
    if (!this.features.search || !this.searchIndex.available) {
      const reason = !this.features.search
        ? "会话搜索已关闭（features.search=false）"
  ...
  sessionRoot(sessionId: string): string {
    return this.layout.sessionRoot(sessionId);
  }

  jsonlPath(sessionId: string): string {
    return this.layout.eventsPath(sessionId);
  }
  ...
  metaPath(sessionId: string): string {
    return this.layout.metaPath(sessionId);
  }
  ...
  getMicroTurn(sessionId: string): number {
    const n = this.readMeta(sessionId)?.micro_turn;
    return typeof n === "number" && Number.isFinite(n) && n > 0 ? Math.trunc(n) : 0;
  }

  setMicroTurn(sessionId: string, turn: number): void {
    const n = Math.max(0, Math.trunc(turn));
    this.patchMeta(sessionId, { micro_turn: n });
  }
  ...
  recoverCold(sessionId: string): void {
    if (!this.features.toolRecovery) return;
  ...
  list(): SessionListItem[] {
    ...
    if (this.features.listCache) writeListCache(this.sessionDir, sessions);
    return sessions;
  }
  ...
  deleteSession(sessionId: string): DeleteResult {
    ...
    if (this.features.listCache) removeListCacheItem(this.sessionDir, sessionId);
  ...
  getLlmHistoryMessages(sessionId: string): SessionMessage[] {
    ...
    return filterLlmVisible(this.branchOf(all, this.getLeafId(sessionId)));
  }

  foldBranch(sessionId: string, opts?: { writeCache?: boolean }): SessionData | null {
    const meta = this.readMeta(sessionId);
    if (!meta) return null;
    const writeCache = opts?.writeCache !== false && this.features.foldCache;
    ...
    const cached = this.features.foldCache ? readFoldCache(this.sessionRoot(sessionId)) : null;
    if (cached && foldCacheUsable(cached, lastSeq, tailFp, gen) && cached.lastSeq === lastSeq) {
      ...
      const branch = this.branchOf(cachedMsgs, meta.leaf_id);
    ...
    if (cached && foldCacheUsable(cached, lastSeq, tailFp, gen) && cached.lastSeq < lastSeq) {
      ...
      return this.sessionFromMeta(meta, this.branchOf(all, meta.leaf_id), traces);
    }
    const located = this.collectLocatedMessages(sessionId);
    const all = located.map((row) => row.message);
    const branch = this.branchOf(all, meta.leaf_id);
  ...
  private touchListCache(sessionId: string, meta?: SessionMeta | null): void {
    if (!this.features.listCache) return;
