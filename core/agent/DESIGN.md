...
- [x] 工具解析使用流式，但工具流式到一半断掉如何执行？ ⚠️ 2026-08 现状：abort 保留 partial + 中断标记；缺必填参数拦截（invalid_args）；未知工具/模式拒绝；同轮资源冲突跳过并 tool_result 报错；orphan tool_call 由 context injectPending/repairOrphaned 补位。完整「删除半截 tool JSON 字段当未调用」的流式 parser 级策略仍主要在 LLM 层 repair，非 Agent 再解析半截帧。
    - [x] 包括：模型调了工具但没给参数，没有的工具，空回复，或参数类型错误 ⚠️ 缺参/未知工具/空响应（监督模式 continue）已处理；参数类型错误依赖工具内校验 + createToolResponse/toolFail
- [x] 网络和模型调用问题
    - [x] 413错误代表超过上下文，这个时候是自动进行压缩后重试 ⚠️ runtime 已实现：出征前≥92%预压 + API 溢出后 forceShrink 最多 3 次再同调用重试
...
- [x] 工具问题
    ...
    - [x] 并行工具冲突，检测到多个工具在同时操作同一个资源，会返回错误信息给模型 ⚠️ 2026-08：同轮 findSameRoundResourceConflicts（写 path / 相同 terminal 命令）后者 skip + tool_result precondition/resource_conflict；跨 agent 资源锁仍无
- [x] loop问题和模型输出问题
    - [x] 在content输出中流式中断，会在最后字后面加上标志 ⚠️ abort 时正文追加「生成被中断」标记；不自动续写（用户意图）
    - [x] 模型输出到 max_tokens 被截断，让ai继续生成 ⚠️ 2026-08：normalizeStopReason/needsContinuation；截断标记 + `<continue>` 续写最多 MAX_LENGTH_CONTINUATIONS=3
    - [x] 模型返回了空的 content 且无 tool_call JSON，直接重试 ⚠️ unusable 原样重试 MODEL_RETRIES=2；监督模式空转 MAX_EMPTY_RETRIES
    - [x] 检测到输出死循环 / 工具重复 ⚠️ 2026-08：runtime-recovery.detectRepeatedToolLoop（工具签名）注入催促最多 3 次；内容级 20 次重复检测仍未做
    - [x] agent规定的单轮内loop最大次数达到限制 ⚠️ agent.json round_limit；超限事件
- [x] 上下文问题
    - [x] 413 压缩后重试同一次调用 ⚠️ 已实现（预压 + 最多 3 次 shrink 重试）
    - [x] 压缩的模型错误降级 ⚠️ summarizer 失败 → truncate/fallbackSummary
    - [x] tool_call 无 tool_result 自动补位 ⚠️ injectPendingToolInterrupts + repairOrphanedToolCalls
    - [x] session 文件写入失败 ⚠️ atomic-write + 吞错保主流程
- [ ] 系统问题：（仍属能力缺口，非本轮范围）
    - [ ] 内存不足、硬盘满、CPU满
    - [ ] 内部服务崩溃自动重启（LSP/浏览器）
    - [ ] 独立定时网络健康检查（现为 LLM 失败后被动探测）
    - [x] hook脚本执行失败不中断 agent
- 
