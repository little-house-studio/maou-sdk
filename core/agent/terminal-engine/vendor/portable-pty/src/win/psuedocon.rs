    ...
    pub fn spawn_command(&self, cmd: CommandBuilder) -> anyhow::Result<WinChild> {
        ...
        let res = unsafe {
            CreateProcessW(
                ...
                ptr::null_mut(),
                ptr::null_mut(),
                // MUST be TRUE: ConPTY inherits the pseudo-console handles via
                // PROC_THREAD_ATTRIBUTE_PSEUDOCONSOLE. bInheritHandles=FALSE
                // causes the child to start without a usable console host and
                // immediately die with STATUS_DLL_NOT_FOUND (0xC0000135) on some
                // Windows builds when OpenConsole/conhost cannot load.
                1,
