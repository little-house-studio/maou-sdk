 ...
 *
 * 实现 ProtocolAdapter 接口，将统一消息格式转换为 Google Vertex AI 格式。
 * Vertex AI 与 Gemini API 使用几乎相同的请求/响应格式（contents/parts 结构、
 * functionCall、thinkingConfig 等），区别主要在于：
 *   1. 认证使用 `Authorization: Bearer {key}`（key 为 GCP access token），
 *      而非 Gemini 的 `x-goog-api-key`。
 *   2. URL 端点不同（由调用方根据 preset.project / preset.location 拼接）。
 *
 * 因此本适配器复用 Gemini 的全部逻辑，仅覆盖 buildRequestHeaders 与 protocolName。
 *
...
export class GoogleVertexAdapter implements ProtocolAdapter {
  readonly protocolName = "google-vertex";

  buildRequestHeaders(preset: APIPreset): Record<string, string> {
    // Vertex AI 使用 GCP access token 作为 Bearer 凭证
    return {
  ...
  /**
   * 将 OpenAI 风格的消息数组转换为 Vertex/Gemini contents 格式
   * 使用 "user" / "model" 两种角色（assistant → model）
   */
  ...
  /** 将 OpenAI 格式的多模态内容转换为 Vertex/Gemini 格式 */
  ...
  buildRequestPayload(params: {
    ...
    // 注入 reasoning_params —— Vertex/Gemini 用 thinkingConfig 控制思考
  ...
  collectToolCalls(
    ...
    for (const [index, chunk] of sorted) {
      ...
      if (!name) continue;
      toolCalls.push({
        id: chunk.id || `vertex_call_${index}`,
        name,
