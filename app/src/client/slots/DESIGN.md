...
- Host：`host/live-slots.tsx` / `host/draft-slots.tsx` 往已声明座位投稿。左右栏内置页签和扩展都走 `registerAsideTab`（座位未声明时晚绑定）。模块顶层 `registerSlotPlugin({ id, apply })`，宿主建表时一并 apply；也可以 `createLiveHostSlots([plugin])`。
