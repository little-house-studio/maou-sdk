...
- 座位树（`map.ts`）：壳层 topbar / body / bottom / overlay；body 内 sidebar / center / files / activity；左右活动条在 LeftAside / RightAside；对话列 trail / messages / permit / composer（chain）/ overlay；输入卡 queue / bar / footer；输入条 overlay / left / plan / approval-mode / model / usage / right。
