...
export function AskScrollRail({ scrollRef, revision }: AskScrollRailProps) {
  ...
  const measure = useCallback(() => {
    ...
    setRailBox({ top: ASK_TRACK_INSET, height: track });
    ...
    setThumb(next ? { ...next, bottom: next.top + next.height } : null);
    setMarks(shown ? collectMarks(el, track) : []);
  }, [scrollRef]);
  ...
  const jumpTo = useCallback(
    (offsetTop: number) => {
      const el = scrollRef.current;
      if (!el) return;
      el.scrollTop = scrollTopToAlignMessage(
        offsetTop,
