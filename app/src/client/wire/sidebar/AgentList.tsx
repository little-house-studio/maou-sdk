...
import { isHiddenManagerAgent, STATUS_LABEL_ZH } from "./agent-tree";
...
export function buildImInbox(agents: readonly DraftAgent[]): DraftAgent[] {
  return agents
    .filter((a) => !a.stale && !isHiddenManagerAgent(a.name, a.displayName))
    .slice()
    .sort((a, b) => {
...
export function buildSystemGroups(agents: DraftAgent[]): HostOrProjectGroup[] {
  const roots = agents.filter(
    (a) =>
      a.group === "system" &&
      !a.parent &&
      !isHiddenManagerAgent(a.name, a.displayName),
  );
...
export function AgentList({
  ...
  const switchPresence = usePresence(switchOpen && canSwitch);
  ...
  useLayoutEffect(() => {
    if (!switchOpen || !canSwitch) return;
      ...
      {tab === "im" ? (
        ...
        <div className="wire-agent-desk" ref={deskRef}>
          <div className={`wire-agent-switch${canSwitch ? "" : " is-solo"}`}>
            ...
            {switchPresence.shown && menuBox ? (
              <div
                className="wire-agent-switch-menu"
                role="listbox"
                aria-label="工作区"
                aria-hidden={switchPresence.phase !== "open"}
                inert={switchPresence.phase !== "open" ? true : undefined}
                {...presenceProps(switchPresence)}
                style={{
                  top: menuBox.top,
                  left: menuBox.left,
                  width: menuBox.width,
